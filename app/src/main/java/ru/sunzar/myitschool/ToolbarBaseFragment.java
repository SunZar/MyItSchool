    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        StocksData.getCurrency(StocksData.Currency.RUB).observe(
                getViewLifecycleOwner(), rub -> baseBinding.rub.setText(
                        String.format(Locale.ENGLISH, "%.4f", rub)
                )
        );
        StocksData.getCurrency(StocksData.Currency.BTC).observe(
                getViewLifecycleOwner(), btc -> baseBinding.btc.setText(
                        String.format(Locale.ENGLISH, "%.4f", btc)
                )
        );
    }
