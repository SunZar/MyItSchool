...
//    }
    ...
    private void onClickButtonUpgradeAddMoney() {
        if (StocksData.eth_count >= MiningData.btc_add * 20) {
            StocksData.eth_count -= MiningData.btc_add * 20;
            MiningData.btc_add += 0.005f;
            updateUpgradeAddMoney();
            updateBtcCountMining();
    ...
    private void onClickButtonUpgradeOfflineMoney() {
        if (StocksData.eth_count >= (MiningData.btc_offline_count + 0.005f) * 200) {
            StocksData.eth_count -= (MiningData.btc_offline_count + 0.005f) * 200;
            MiningData.btc_offline_count += 0.005f;
            updateUpgradeAddOfflineMoney();
            updateBtcCountMining();
            Toast.makeText(this.getActivity(), "Улучшение куплено!", Toast.LENGTH_SHORT).show();
        } else {
    ...
    private void onClickButtonAddOfflineMoneyCount() {
        if (StocksData.eth_count >= (101 - MiningData.btc_offline_time * 10) * 1000 && MiningData.btc_offline_time >= 5.5f) {
            StocksData.eth_count -= (101 - MiningData.btc_offline_time * 10) * 1000;
    ...
    private void onClickButtonBuyVideocard() {
        if (StocksData.eth_count >= MiningData.price_buy_videocard) {
            StocksData.eth_count -= MiningData.price_buy_videocard;
    ...
    private void updateBtcCountMining() {
        binding.btcCount.setText("BTC: " + StocksData.eth_count);
        MiningData.price_buy_videocard = (float) Math.floor(StocksData.eth_count * 0.5f * 100) / 100;
        updateButtonBuyVideocard();
    }
    ...
    private void updateButtonBuyVideocard() {
        binding.priceBuyVideocard.setText("Цена: " + MiningData.price_buy_videocard);
    }
...
//        mySP = this.getPreferences(MODE_PRIVATE);
        ...
        MiningData.btc_add = sharedMiningData.getFloat("btc_add_pref", 0.005f);
